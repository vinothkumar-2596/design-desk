...
const Toaster = ({ ...props }: ToasterProps) => {
  ...
  const toastBaseClass = isDark
    ...
    : "group toast rounded-2xl border border-[#D7E0F8] bg-[rgba(255,255,255,0.96)] text-[#162858] backdrop-blur-md shadow-[0_18px_45px_-30px_rgba(37,99,235,0.28)] ring-1 ring-white/70";
  const toastTitleClass = isDark ? "font-sans text-slate-100" : "font-sans text-[#162858]";
  const toastDescriptionClass = isDark
    ? "font-sans text-slate-300"
    : "font-sans text-[#5C6E95]";
    ...
    <Sonner
      ...
      toastOptions={{
        classNames: {
          ...
          error: isDark
            ? "border-rose-400/45"
            : "border-[#F2BBC4] bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(255,245,247,0.98))]",
          success: isDark
            ? "border-[#1E3A75]/70"
